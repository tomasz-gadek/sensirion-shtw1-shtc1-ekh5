...
// Measurement additional delay in miliseconds, at least 200
#define MEAS_DELAY_MS 250
#define UPDATE_ITERATION_NUMBER 4
#define NUMBER_OF_PLOT_POINTS 100
...
void PrintResultPlots(struct tm tm)
{
	...
	//fprintf(gnuplot, "set yrange [-40:100]\n");
...
void UpdatePlotsOneWindow(struct tm tm, FILE *gnuplot)
{
	char file_path_string[30];	
	strftime(file_path_string, 30, "records/%Y_%b_%d_%H_%M_%S\0", &tm);
	...
	fprintf(gnuplot, "set title 'On-line measurement plots.'\n");
	fprintf(gnuplot, "set multiplot layout 3,1 rowsfirst\n");
	//fprintf(gnuplot, "stats [1:3] '<tail -n %i %s'\n", NUMBER_OF_PLOT_POINTS, file_path_string);
	fprintf(gnuplot, "set title 'Temperature plot.'\n");				
	fprintf(gnuplot, "set xlabel 'Time[s]'\n");
	fprintf(gnuplot, "set ylabel 'Temperature [*C]' rotate\n");
	fprintf(gnuplot, "set yrange [-40:100]\n");
	//fprintf(gnuplot, "set xrange [stats_min_y:stats_max_y]\n");	
	fprintf(gnuplot, "plot '<tail -n %i %s' using 1:2 title 'temperature' with lines\n", NUMBER_OF_PLOT_POINTS, file_path_string);
	fprintf(gnuplot, "set xrange [GPVAL_DATA_X_MIN:GPVAL_DATA_X_MAX]\n");
	...
	fprintf(gnuplot, "plot '<tail -n %i %s' using 1:3 title 'humidity' with lines\n", NUMBER_OF_PLOT_POINTS, file_path_string);
	fprintf(gnuplot, "set xrange [GPVAL_DATA_X_MIN:GPVAL_DATA_X_MAX]\n");
	...
	fprintf(gnuplot, "set ylabel 'Dew_Point[*C]' rotate\n");
	fprintf(gnuplot, "set yrange [-0:100]\n");
	fprintf(gnuplot, "plot '<tail -n %i %s' using 1:4 title 'dew_point' with lines\n", NUMBER_OF_PLOT_POINTS, file_path_string);
	fprintf(gnuplot, "set xrange [GPVAL_DATA_X_MIN:GPVAL_DATA_X_MAX]\n");
